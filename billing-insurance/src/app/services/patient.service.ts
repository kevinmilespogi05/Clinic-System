...
export class PatientService {
  ...
  getPatients(): Observable<any> {
    return this.http
      .get<any>(`${this.baseUrl}/api/users/getUsers.php`)
  ...
  getAppointments() {
    ...
    return this.http.get<any>(
      `${this.baseUrl}/api/appointments/get_appointments.php?role=${role}&id=${id}`
    );
  }
  ...
  updateAppointmentStatus(
    ...
    return this.http
      .post<any>(`${this.baseUrl}/api/appointments/update_status.php`, {
        id: appointmentId,
        status: status,
      })
      .pipe(catchError(this.handleError));
  ...
  declineAppointment(appointmentId: number): Observable<any> {
    return this.http
      .post<any>(`${this.baseUrl}/api/appointments/decline.php`, {
        id: appointmentId,
      })
      .pipe(catchError(this.handleError));
  }
      ...
      this.getRequest('api/appointments/stats.php').subscribe(
        (appointmentsStats) => {
          this.getRequest('api/billing/stats.php').subscribe(
            (billingStats) => {
              this.getRequest('api/insurance/stats.php').subscribe(
                // New request to fetch the stats
                (stats) => {
                  const combinedStats = {
  ...
  getUserByUsername(username: string): Observable<any> {
    return this.postRequest('api/billing/get_user_by_username.php', {
      username,
    });
  }
  ...
  getTransactions(): Observable<any> {
    return this.http
      .get<any>(`${this.baseUrl}/api/billing/get_transactions.php`)
      .pipe(catchError(this.handleError));
  }
  ...
  getInsuranceClaims(userId: number, isAdmin: number = 0): Observable<any[]> {
    return this.getRequest(
      `api/insurance/read_all.php?user_id=${userId}&is_admin=${isAdmin}`
    );
  }
  ...
  private postRequest(endpoint: string, payload: any): Observable<any> {
    return this.http
      .post<any>(`${this.baseUrl}/${endpoint}`, payload)
      .pipe(catchError(this.handleError));
  }

  private putRequest(endpoint: string, payload: any): Observable<any> {
    return this.http
      .put<any>(`${this.baseUrl}/${endpoint}`, payload)
      .pipe(catchError(this.handleError));
  }
