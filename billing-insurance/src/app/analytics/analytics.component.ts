...
export class AnalyticsComponent implements OnInit {
  ...
  renderCharts(stats: any): void {
    ...
    setTimeout(() => {
      ...
      if (!ctxAppointments || !ctxInsurance) {
        console.error(
...
this.insuranceClaims.forEach((claim) => {
  ...
  // Finalize the PDF for preview
  addFooter(pageNum);
  const pdfUrl = doc.output('bloburl'); // Generate a blob URL for the PDF
  window.open(pdfUrl, '_blank'); // Open the PDF in a new tab for preview

  // Notify the user
  Swal.fire('Preview Ready', 'The report preview has been opened in a new tab.', 'info');
}
}